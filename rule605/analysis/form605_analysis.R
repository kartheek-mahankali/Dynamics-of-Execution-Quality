...
#18MAR2023 Changes (@Kartheek):

# defined the function get_weighted_quantiles which takes f605_mktble_df,
# order type and order size as inputs and gives us df_sz_all_w
...
##@@Kartheek : Above statements are replaced by below funtion
...
#  otype = "mktbl_lmt_ordr"
#} else if (ordertype == 13) {
#  otype = "insd_qt_lmt_ordr"
#} else if (ordertype == 14) {
#  otype = "at_qt_lmt_ordr"
#} else if (ordertype == 15) {
#  otype = "nr_qt_lmt_ordr"
#}
#
#if (ordersize == 21) {
#  osize = "100-499"
#} else if (ordersize == 22) {
#  osize = "500-1999"
#} else if (ordersize == 23) {
#  osize = "2000-4999"
#} else if (ordersize == 24) {
#  osize = "5000+"
#} 

#get_weighted_quantiles <- function(df, otype, osize) {
#  df_sz_499 <- df %>% filter(order_type == otype, order_size == osize)
#  df_sz_499_w <- t(data.frame(wtd.quantile(df_sz_499$net_pi_mcaway, df_sz_499$mcaway_exec_shrs)))
#  rownames(df_sz_499_w) <- osize
#  colnames(df_sz_499_w) <- c("0%", "25%", "50%", "75%", "100%")
#  df_sz_all_w <- data.frame(rbind(df_sz_499_w))
#  colnames(df_sz_all_w) <- c("0%", "25%", "50%", "75%", "100%")
...
#get_weighted_quantiles <- function(df, otype, osize) {
#  df_sz <- df %>% filter(order_type == otype, order_size == osize)
#  df_sz_w <- t(data.frame(wtd.quantile(df_sz$net_pi_mcaway, df_sz$mcaway_exec_shrs)))
#  rownames(df_sz_w) <- osize
#  colnames(df_sz_w) <- c("0%", "25%", "50%", "75%", "100%")
#  return(df_sz_w)
#}
#df_sz_w <- get_weighted_quantiles(f605_mktble_df, otype, osize)
#
#df_sz_all_w <- data.frame(rbind(df_sz_w))
#colnames(df_sz_all_w) <- c("0%", "25%", "50%", "75%", "100%")
#df_sz_all_w$order_size <- ordered(rownames(df_sz_all_w), levels = osize)
